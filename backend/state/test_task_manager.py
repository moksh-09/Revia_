...
def test_task_manager_agent_brain_usage():
    ...
    # 2. Create a task
    task1 = manager.create_task("What are the Q1 sales?")
    ...
    task2 = manager.create_task("Wait, what about Q2 sales?")
    ...
    stale_result = manager.check_fence(task1.fence_token)
    assert stale_result == "rejected_stale"
    
    # Simulate a valid fence_token check (from the second, active task)
    valid_result = manager.check_fence(task2.fence_token)
